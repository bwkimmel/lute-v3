...
def analyze_word(term, features, termrepo, language_id, patterns, undefined_roots, undefined_patterns):
    ...
    if pattern in patterns:
        ...
        if term.translation is not None and term.translation != p.translation:
            ...
            return
        _trace(term.text, "updating term: status={}, parent={}, translation={}", root_term.status, root_word, translation)
        term.status = root_term.status
        term.parents = [root_word]
...
def analyze_book(book, collector, termrepo, language_id, mecab, patterns, undefined_roots, undefined_patterns):  # pylint: disable=too-many-locals
    ...
    morphemes = mecab.parse(fulltext)
    ...
    for morpheme in morphemes:
        if morpheme.feature.pos[0] == 'S' and morpheme.feature.pos not in ['SL', 'SN']: # punctuation
            continue
...
#            if not termrepo.find(language_id, k):
            # print("Analyzing word: {}, bad={}".format(cur_word, bad_word))
            if bad_word:
                _trace(cur_word, "word has bad feature: {}", cur_word_features)
            if not bad_word:
                _trace(cur_word, "looking up word")
                term = termrepo.find_or_new(language_id, cur_word)
                # print("  > Term: {}".format(term))
                if 'fixme' in term.term_tags and term.parents and 'generated' not in term.term_tags:
                    _trace(cur_word, "resetting word with fixme tag")
                ...
                if term.id is not None and 'generated' in term.term_tags:
                    _trace(cur_word, "generated word already exists, resetting...")
                    term.status = 0 
                    term.translation = None
                    term.term_tags = []
                    term.parents = []
                ...
                if term.id is None or (term.translation is None and term.status != 98) or 'generated' in term.term_tags:
                    _trace(cur_word, "analyzing word")
                # if term.translation is None and term.status != 98:
                    # print("  > processing")
                    analyze_word(term, cur_word_features, termrepo, language_id, patterns, undefined_roots, undefined_patterns)
                    _trace(cur_word, "translated word as '{}'", term.translation)
...
def run(todo_file, include_books, exclude_books, commit, trace):
    global _TRACE_WORDS
    if trace:
    ...
    books = [
        ...
        if b.language.name == language_name
            ...
            and (not include_books or b.id in include_books)
    ]
    if len(books) == 0:
    ...
    with pattern_file.open(newline='') as f:
        ...
        for row in r:
            ...
            _trace(signature, "defining pattern: translation={}, tags={}", translation, tags)
    ...
    for term in terms.values():
        ...
        if len(term.parents) != 1:
            print("Word {} has invalid number of parents: [{}]".format(term.text, ", ".join(term.parents)))
            _trace(term.text, "invalid number of parents")
        ...
        if not term.translation:
            ...
            continue
        num_added += 1
        _trace(term.text, "added term with parent={}, translation={}", term.parents[0], term.translation)
        repo.add(term)

    print("Undefined patterns:")
    print("pattern,refs,num_roots")
    for stats in undefined_patterns.values():
        print("{},{},{}".format(stats.pattern, stats.refs, len(stats.roots)))
    ...
    print("Done.")
    print("Stats: conflicts={}, invalid={}, untranslatable={}, unknown roots={}, added={}, deleted={}, removed_fixme_tag={}".format(
        num_conflicts, num_invalid_parent_count, num_untranslatable, len(undefined_roots), num_added, num_deleted, num_patched_parents,
    ))
